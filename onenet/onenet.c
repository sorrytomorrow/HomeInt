...
#define PROID			"ZbWW400uYq"

#define ACCESS_KEY		"SEVmUk5zRDZVYmZQN05jd05sNkJXRkh1VHFBOU5XaGo="
...
extern u8 D1_State_W,D2_State_W,Light_State,Stepper_State,Mode;
extern u8 temp,humi;
extern int light_v,gas;
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"door\":{\"value\":%s},", D1_State_W ? "true":"false");
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"windows\":{\"value\":%s},", D2_State_W ? "true":"false");
	strcat(buf, text);
...
//	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"curtain\":{\"value\":%s},",Stepper_State ? "true":"false");
	strcat(buf, text);
//	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"lightA\":{\"value\":%s},",Light_State ? "true":"false");
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"LightSen\":{\"value\":%d},",light_v);
	strcat(buf, text);
//	
//	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"MODE\":{\"value\":%s}",Mode ? "true":"false");
	strcat(buf, text);
...
extern u8 D1_State_W,D2_State_W,Light_State,Stepper_State,Stepper_num,Mode;
void OneNet_RevPro(unsigned char *cmd)
{
	...
	char numBuf[10];
	int num = 0;
	cJSON *raw_json,*params_json,*door_data,*windows_data,*Light_data,*curtain_data,*mode_data;
...
//				}
				...
				raw_json = cJSON_Parse(req_payload);    //把指向cJSON这个结构体的指针传递给raw_json
				params_json = cJSON_GetObjectItem(raw_json,"params");
				
				DelayXms(10);
				door_data = cJSON_GetObjectItem(params_json,"door");
				DelayXms(10);
				windows_data = cJSON_GetObjectItem(params_json,"windows");
				DelayXms(10);
				Light_data = cJSON_GetObjectItem(params_json,"lightA");
				DelayXms(10);
				curtain_data = cJSON_GetObjectItem(params_json,"curtain");
				DelayXms(10);
				mode_data = cJSON_GetObjectItem(params_json,"MODE");
				if(door_data != NULL)
				{
					if(door_data->type == cJSON_True) 
					{
						D1_State_W=1;UsartPrintf(USART_DEBUG,"大门已经打开");
					}
					else{D1_State_W=0;UsartPrintf(USART_DEBUG,"大门已经关闭");}
				...
				{
					if(windows_data->type == cJSON_True) 
					{
						D2_State_W=1;UsartPrintf(USART_DEBUG,"窗户已经打开");
					}
					else{D2_State_W=0;UsartPrintf(USART_DEBUG,"窗户已经关闭");}
				...
				if(Light_data != NULL)
				{
					if(Light_data->type == cJSON_True) 
					{
						Light_State=1;UsartPrintf(USART_DEBUG,"灯光已经打开");
					}
					else{Light_State=0;UsartPrintf(USART_DEBUG,"灯光已经关闭");}
				...
				{
					if(curtain_data->type == cJSON_True) 
					{
						Stepper_State=1;UsartPrintf(USART_DEBUG,"窗帘已经打开");Stepper_num=1;
					}
					else{Stepper_State=0;UsartPrintf(USART_DEBUG,"窗帘已经关闭");Stepper_num=1;}
				...
				if(mode_data != NULL)
				{
					if(mode_data->type == cJSON_True) 
					{
						Mode=1;UsartPrintf(USART_DEBUG,"自动模式已经打开");
					}
					else{Mode=0;UsartPrintf(USART_DEBUG,"自动模式已经关闭");}
...
//				if(ledB_data != NULL)
//				{
//					if(ledB_data->type == cJSON_True) 
//					{
//						UsartPrintf(USART_DEBUG,"111111111111111111111111111");
//					}
//					
//				}
